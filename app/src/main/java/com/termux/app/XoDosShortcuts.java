...
if (parent instanceof ScrollView) {
    ...
    private void launchShortcut(String name, String execPath, String wineType) {
        ...
        try {
            /*
             * REXX AutoTune:
             * Detect the phone's RAM, CPU and display and expose conservative
             * values to the existing XoDos Wine/Box64 launcher.
             *
             * IMPORTANT:
             * We keep XoDos' existing execPath, Wine selection, DXVK and
             * driver handling. This layer only adds launch-time hints.
             */
            RexxProfile profile = getRexxProfile(name);

            String dxvk = getAppDxvk(name);
            String driver = getAppDriver(name);
            String savedCores = getAppCores(name);

            String cores = savedCores;
            if (cores == null || cores.trim().isEmpty() || "0".equals(cores)) {
                cores = String.valueOf(profile.cores);
            }

            String coreMask = getTasksetMask(cores);

            String finalExecPath = execPath;
            if (coreMask != null && !coreMask.isEmpty()) {
                finalExecPath = "taskset -c " + coreMask + " " + execPath;
            }

            String safeAppName = sanitizeFileName(name);
            String logFile = termuxHome + "/rexx_" + safeAppName + ".log";

            StringBuilder command = new StringBuilder();

            command.append("export DISPLAY=:0; ");
            command.append("export PREFIX='").append(shellEscape(USR_PREFIX)).append("'; ");
            command.append("export HOME='").append(shellEscape(termuxHome)).append("'; ");
            command.append("export TMPDIR='").append(shellEscape(USR_PREFIX + "/tmp")).append("'; ");

            // REXX launch hints. Existing XoDos scripts may consume these.
            command.append("export REXX_APP='").append(shellEscape(name)).append("'; ");
            command.append("export REXX_WIDTH=").append(profile.width).append("; ");
            command.append("export REXX_HEIGHT=").append(profile.height).append("; ");
            command.append("export REXX_RAM_MB=").append(profile.ramMb).append("; ");
            command.append("export REXX_CORES=").append(cores).append("; ");
            command.append("export REXX_PRESET=").append(profile.preset).append("; ");

            // Preserve audio routing for applications that use PulseAudio.
            command.append("export PULSE_SERVER=127.0.0.1; ");

            // Start X11 without killing an already running X11 server.
            command.append(
            ...
            /*
             * Do NOT pkill all Wine processes here.
             * The previous implementation could kill another running Windows app.
             */
            command.append(finalExecPath);
            command.append(" > '").append(shellEscape(logFile)).append("' 2>&1");
            command.append("\n");
            ...
            Log.d(
                    ...
                    "Launching " + name +
                            ...
                            " | RAM=" + profile.ramMb + "MB" +
                            " | Cores=" + cores +
                            " | Preset=" + profile.preset
            );

            Log.d("LaunchCommand", "Executing: " + finalCommand);
            ...
            // Fallback if no active Termux terminal session exists.
            launchWithProcessBuilder(name, finalExecPath);

        } catch (Exception e) {
            Log.e("LaunchError", "Failed to launch: " + e.getMessage(), e);
            launchWithProcessBuilder(name, execPath);
        }
    }
     ...
     * REXX device profile.
     *
     * This intentionally stays inside XoDosShortcuts.java so no extra Java
     * class is required for this first integration.
     */
    private RexxProfile getRexxProfile(String appName) {
        SharedPreferences prefs =
                mContext.getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE);

        try {
            long totalRamMb = 4096;
            ...
            boolean lowRam = totalRamMb <= 4608;
            ...
            if (lowRam) {
                ...
                width = Math.min(dm.widthPixels, 1280);
                height = Math.min(dm.heightPixels, 720);
            }
            ...
            /*
             * Save the detected profile per app. If the user later changes
             * the existing XoDos core setting, that setting still wins.
             */
            prefs.edit()
                    .putInt("REXX_WIDTH_" + appName, width)
                    .putInt("REXX_HEIGHT_" + appName, height)
                    .putInt("REXX_RAM_" + appName, wineRam)
                    .putInt("REXX_CORES_" + appName, recommendedCores)
                    .putString("REXX_PRESET_" + appName, preset)
                    .apply();

            return new RexxProfile(
                    width,
                    height,
                    wineRam,
                    recommendedCores,
            ...
            Log.e(
                    "REXX_AUTO",
                    "AutoTune detection failed, using safe defaults",
                    e
            );
    ...
    private static final class RexxProfile {
        final int width;
        final int height;
        final int ramMb;
        final int cores;
    ...
    /**
     * Convert a core count into a taskset CPU list.
     * Existing XoDos manual core settings are respected.
     */
    private String getTasksetMask(String cores) {
        if (cores == null ||
                cores.trim().isEmpty() ||
                "0".equals(cores)) {
            return null;
        ...
        try {
            int count = Integer.parseInt(cores.trim());
    ...
    /**
     * Monitor X11 for a short period after launch.
     * Uses XoDos' existing Termux terminal session.
     */
    private void monitorX11Launch(
            String appName,
            TerminalSession session
    ) {
        new Thread(() -> {
            try {
                ...
                for (int i = 0; i < MAX_CHECKS; i++) {
                    ...
                    if (session == null || !session.isRunning()) {
                        Log.w(
                                "REXX_X11",
                                "Termux session stopped while monitoring " +
                                        appName
                        );
                        return;
                    }
                    ...
                    String checkFile =
                            termuxHome + "/.rexx_x11_check";

                    session.write(
                            "DISPLAY=:0 xdotool search --onlyvisible --name . > '" +
                                    shellEscape(checkFile) +
                                    "' 2>/dev/null\n"
                    );
                ...
                if (!appVisible) {
                    Log.w(
                            "REXX_X11",
                            "No visible X11 window detected for " +
                                    appName
                    );
